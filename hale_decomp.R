...
lx_to_dx <- function(dt, id_vars = NULL, terminal_age = 95, assert_na = T) {
  setorder(dt, "age")
...
gen_nLx <- function(dt, id_vars = NULL, terminal_age = 95, assert_na = T) {
  id_vars <- setdiff(id_vars, "age")
  setorderv(dt, c(id_vars, "age"), order = 1)
...
get_full_life_table <- function(data_table, by_vars = NULL){
  ...
  data_table2[age_group_id != 235, lx := cumprod(px)/ px, by = by_vars]
  data_table2[, term_lx := .SD[age_group_id != 235, prod(px)], by = by_vars]
  data_table2[age_group_id == 235, lx := term_lx]
...
dt <- melt(data_t, id.vars = id_vars, measure.vars=c("acmr", "cr_ylds", "crmr", "ylds"))
dt[, cause_replaced := as.integer(variable %like% "^cr")]
dt[, variable := ifelse(variable %like% "mr$", "mx", "ylds")]
dt <- dcast(dt, as.formula(paste0(paste(c(id_vars, "cause_replaced"), collapse=" + "), " ~ variable")),
            value.var = "value")

dt <- dt[, .(cause_id, draw, sex_id, year, cause_replaced, age_group_id, age, age_length, mx, ylds)]
...
dt[, c("ax", "mx", "qx") := NULL]
dt <- merge(lt, dt, by=c(id_vars, "age_group_id"))

dt <- merge(dt, data_t[, c(setdiff(id_vars, "cause_replaced"), "age_group_id", "cases"), with=FALSE],
            by=c(setdiff(id_vars, "cause_replaced"), "age_group_id"))
dt[, haL := nLx * ylds / cases]
setorder(dt, c(id_vars, "age"), order = 1)
...
setcolorder(dt, c("cause_id", "gbd_cause_name", "dex_cause_id", "dex_cause_name",
                  ...
                  "hale", "hale_cause_replaced", "ex", "ex_cause_replaced", "ylds", "ylds_cause_replaced"))
...
dt <- dt[, lapply(.SD, mean), by=.(cause_id, gbd_cause_name, dex_cause_id, dex_cause_name, year),
         .SDcols=c("hale_effect", "hale", "hale_cause_replaced", "ex", "ex_cause_replaced", "ylds", "ylds_cause_replaced")]
