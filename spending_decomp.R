...
# setnames(data_t, c("expenditure", "all_cause_expenditure"), c("cs_expenditure", "expenditure"))
setnames(data_t, "expenditure", "cs_expenditure")

dt <- melt(data_t, id.vars = id_vars, measure.vars=c("acmr", "cr_expenditure", "crmr", "all_cause_expenditure"))
dt[, cause_replaced := as.integer(variable %like% "^cr")]
dt[, variable := ifelse(variable %like% "mr$", "mx", "all_cause_expenditure")]
dt <- dcast(dt, as.formula(paste0(paste(c(id_vars, "cause_replaced"), collapse=" + "), " ~ variable")),
            value.var = "value")

dt <- dt[, .(cause_id, draw, sex_id, year, cause_replaced, age_group_id, age, age_length, mx, all_cause_expenditure)]
...
##spending: cum sum of evryone older than x /no. of survivors beyond x
...
dt[, haL := nLx * (all_cause_expenditure / population)]

setorderv(dt, c(id_vars, "age"), order = -1)
dt[, spending := cumsum(haL)/lx, by=id_vars]
setorderv(dt, c(id_vars, "age"), order = 1)
...
dt <- dcast(dt,
            ...
            value.var=c("ex", "spending", "all_cause_expenditure"))

setnames(dt, names(dt), gsub("_1$", "_cause_replaced", names(dt)))
setnames(dt, names(dt), gsub("_0$", "", names(dt)))

input_data <- fread("./spending_decomp_input_data_adjusted.csv")
...
setcolorder(dt, c("cause_id", "gbd_cause_name", "dex_cause_id", "dex_cause_name",
                  ...
                  "spending", "spending_cause_replaced", "ex", "ex_cause_replaced", "all_cause_expenditure", "all_cause_expenditure_cause_replaced"))
fwrite(dt, "./spending_cause_replaced_life_table.csv")

dt <- dt[age_group_id == 0]
dt[, age_group_id := NULL]
dt[, spending_effect := spending_cause_replaced - spending]

fwrite(dt, "./spending_decomp_results_by_draw.csv")

# creating a data frame containing draw 0 only.
dt_draw_0 <- dt[draw == 0, ]
fwrite(dt_draw_0, "./draw_0_spending_decomp_results_by_draw.csv")

dt <- dt[, lapply(.SD, mean), by=.(cause_id, gbd_cause_name, dex_cause_id, dex_cause_name, year),
         .SDcols=c("spending_effect", "spending", "spending_cause_replaced", "ex", "ex_cause_replaced", "all_cause_expenditure", "all_cause_expenditure_cause_replaced")]
