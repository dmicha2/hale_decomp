...
#drop age group specific extra variables for next part of analysis
dt = subset(dt, select = -c(tot_spending_above_age_group))
...
mean_ui_dt<- dt_0 %>% group_by(cause_id, gbd_cause_name, dex_cause_id, dex_cause_name, sex_id, year) %>% 
  summarise(mean_spending= mean(lt_spending),
            ...
            mean_spending_cause_replaced=mean(cr_lt_spending),
            l_spending_cause_replaced = quantile(cr_lt_spending, na.rm = TRUE, probs=0.025),
            u_spending_cause_replaced = quantile(cr_lt_spending, na.rm = TRUE, probs=0.975),
            mean_ex=mean(ex),
            l_ex = quantile(ex, na.rm = TRUE, probs=0.025),
            u_ex = quantile(ex, na.rm = TRUE, probs=0.975),
            mean_ex_cause_replaced=mean(cr_ex),
            l_ex_cause_replaced = quantile(cr_ex, na.rm = TRUE, probs=0.025),
            u_ex_cause_replaced = quantile(cr_ex, na.rm = TRUE, probs=0.975),
            mean_all_cause_expenditure=mean(ac_spending),
            l_all_cause_expenditure = quantile(ac_spending, na.rm = TRUE, probs=0.025),
            u_all_cause_expenditure = quantile(ac_spending, na.rm = TRUE, probs=0.975),
            mean_all_cause_expenditure_cause_replaced=mean(cr_ac_spending),
            l_all_cause_expenditure_cause_replaced = quantile(cr_ac_spending, na.rm = TRUE, probs=0.025),
            u_all_cause_expenditure_cause_replaced = quantile(cr_ac_spending, na.rm = TRUE, probs=0.975),
