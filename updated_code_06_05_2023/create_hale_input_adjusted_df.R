...
sum_ylds_456 <- sum_ylds %>%
  filter(cause_id ==456) %>% # apply filter condition
  filter(age_group_id %in% c("15", "16", "17", "18", "19", "20", "30", "160")) %>%
  group_by(draw, year_id) %>%
  summarize(sum_ylds = sum(cs_ylds))

dt <- merge(dt, sum_ylds_456, by=c("draw", "year_id"), all=TRUE)
...
# replacing cs_ylds column values with new values in sum_ylds column for specific cause & age-group pairings
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '15',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '16',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '17',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '18',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '19',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '20',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '30',sum_ylds/8,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '456' & age_group_id == '160',sum_ylds/8,cs_ylds))
...
sum_ylds_718 <- sum_ylds %>%
  filter(cause_id ==718) %>% # apply filter condition
  filter(age_group_id %in% c("30", "160")) %>%
  group_by(draw, year_id) %>%
  summarize(sum_ylds = sum(cs_ylds))

dt <- merge(dt, sum_ylds_718, by=c("draw", "year_id"), all=TRUE)
...
# replacing cs_ylds column values with new values in sum_ylds column for specific cause & age-group pairings
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '718' & age_group_id == '30',sum_ylds/2,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '718' & age_group_id == '160',sum_ylds/2,cs_ylds))
...
## For cs_ylds
sum_ylds <- dt[, c("gbd_cause_name", "draw", "age_group_id", "year_id", "cs_ylds")]

sum_ylds_dex_5570 <- sum_ylds %>%
  filter(gbd_cause_name == "dex_agg other neurological disorders") %>% # apply filter condition
  filter(age_group_id %in% c("0", "5")) %>%
  group_by(draw, year_id) %>%
  summarize(sum_ylds = sum(cs_ylds))

dt <- merge(dt, sum_ylds_dex_5570, by=c("draw", "year_id"), all=TRUE)
...
# replacing cs_ylds column values with new values in sum_ylds column for specific cause & age-group pairings
dt <- dt %>% 
  mutate(cs_ylds = ifelse(gbd_cause_name == "dex_agg other neurological disorders" & age_group_id == '0',sum_ylds/2,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(gbd_cause_name == "dex_agg other neurological disorders" & age_group_id == '5',sum_ylds/2,cs_ylds))
...
# replacing cs_ylds column values with new values in sum_ylds column for specific cause & age-group pairings
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '561' & year_id == '2016' & age_group_id == '0',sum_ylds/4,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '561' & year_id == '2016' & age_group_id == '5',sum_ylds/4,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '561' & year_id == '2016' & age_group_id == '6',sum_ylds/4,cs_ylds))
dt <- dt %>% 
  mutate(cs_ylds = ifelse(cause_id == '561' & year_id == '2016' & age_group_id == '7',sum_ylds/4,cs_ylds))
